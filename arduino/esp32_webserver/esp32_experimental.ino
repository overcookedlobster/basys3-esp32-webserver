...
// Adjust these based on your Pmod ESP32 module and connection
#define RX_PIN 16  // Connect to FPGA TX
#define TX_PIN 17  // Connect to FPGA RX (if needed in the future)
...
void handleRoot() {
  ...
  html += "</head>\n";
  html += "<body>\n";
  html += "  <h1>FPGA Button and Switch Status</h1>\n";
  ...
  // Switch section (low byte)
  html += "  <div class='section'>\n";
  html += "    <h2>Switches (0-7)</h2>\n";
  html += "    <div class='container'>\n";
  for (int i = 0; i < 8; i++) {
    bool state = (switches_low & (1 << i)) != 0;
    html += "      <div class='state " + String(state ? "on" : "off") + "'>SW" + String(i) + ": " + String(state ? "On" : "Off") + "</div>\n";
  }
  html += "    </div>\n";
  html += "  </div>\n";

  // Switch section (high byte)
  html += "  <div class='section'>\n";
  html += "    <h2>Switches (8-15)</h2>\n";
  html += "    <div class='container'>\n";
  for (int i = 0; i < 8; i++) {
    bool state = (switches_high & (1 << i)) != 0;
    html += "      <div class='state " + String(state ? "on" : "off") + "'>SW" + String(i+8) + ": " + String(state ? "On" : "Off") + "</div>\n";
  }
  html += "    </div>\n";
  html += "  </div>\n";
...
void loop() {
  ...
  if (Serial1.available() >= 3) {
    ...
    Serial.print(", Switches Low: 0x");
    Serial.print(switches_low, HEX);
    Serial.print(", Switches High: 0x");
    Serial.println(switches_high, HEX);
  }
